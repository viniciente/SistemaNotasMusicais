...
procedure TfrmArquivos.PrepararGridImport;
begin
  cdsImport.Close;
  cdsImport.FieldDefs.Clear;
  cdsImport.FieldDefs.Add('Status',     ftString, 10);
  cdsImport.FieldDefs.Add('Nome',       ftString, 100);
  cdsImport.FieldDefs.Add('Tipo',       ftString, 50);
  cdsImport.FieldDefs.Add('Tonalidade', ftString, 50);
  cdsImport.FieldDefs.Add('Notas',      ftString, 255);
  cdsImport.FieldDefs.Add('Descricao',  ftString, 500);
...
begin
  ...
  try
    ...
    begin
      msgErro := 'Tipo "' + sTipo + '" não encontrado no banco';
      Exit;
    end;
    ...
    if vQuery.IsEmpty then
    begin
      msgErro := 'Tonalidade "' + sTonalidade + '" não encontrada no banco';
      Exit;
    end;
    ...
    vQuery.SQL.Text :=
      ...
      'WHERE LOWER(nome) = LOWER(:nome) AND tonalidadeId = :ton';
    vQuery.ParamByName('nome').AsString := sNome;
    vQuery.ParamByName('ton').AsInteger := tonalidadeId;
    vQuery.Open;
    if vQuery.FieldByName('total').AsInteger > 0 then
    begin
      msgErro := 'Escala "' + sNome + '" já cadastrada com essa tonalidade';
      Exit;
    end;

    // Busca IDs das notas
    notasIds := '';
    notasArr := sNotas.Split([',']);
    for i := 0 to High(notasArr) do
    begin
      notaNome := Trim(notasArr[i]);
      vQuery.Close;
      ...
      if vQuery.IsEmpty then
      begin
        msgErro := 'Nota "' + notaNome + '" não encontrada no banco';
        Exit;
      end;
...
var
  ...
  i: Integer;
  sNome, sTipo, sTonalidade, sNotas, sDescricao, erro, status: string;
begin
  linhas := TStringList.Create;
  try
    ...
    begin
      if Trim(linhas[i]) = '' then Continue; // ignora linhas em branco
      ...
      begin
        cdsImport.Append;
        cdsImport.FieldByName('Status').AsString  := 'ERRO';
        cdsImport.FieldByName('Nome').AsString    := Trim(linhas[i]);
        cdsImport.FieldByName('Erro').AsString    :=
          'Formato inválido: precisa de 5 campos separados por "|"';
        cdsImport.Post;
        Continue;
      end;
      ...
      erro := '';
      if sNome = ''       then erro := 'Nome vazio';
      if sTipo = ''       then erro := erro + IfThen(erro<>'', ' | ') + 'Tipo vazio';
      if sTonalidade = '' then erro := erro + IfThen(erro<>'', ' | ') + 'Tonalidade vazia';
      if sNotas = ''      then erro := erro + IfThen(erro<>'', ' | ') + 'Notas vazias';

      if erro <> '' then status := 'ERRO' else status := 'OK';

      cdsImport.Append;
      cdsImport.FieldByName('Status').AsString     := status;
      cdsImport.FieldByName('Nome').AsString       := sNome;
      cdsImport.FieldByName('Tipo').AsString       := sTipo;
      cdsImport.FieldByName('Tonalidade').AsString := sTonalidade;
      cdsImport.FieldByName('Notas').AsString      := sNotas;
      cdsImport.FieldByName('Descricao').AsString  := sDescricao;
      cdsImport.FieldByName('Erro').AsString       := erro;
      cdsImport.Post;
    end;
